...
def create_camera(pyrender, pyrender_body_mesh, scene, side, camera_location=None):
    ...
    camera_pose = rotate_matrix_y(camera_pose, side)
...
def render(
    ...
    camera_location=np.array([0, 0.97, 4.15])
    create_camera(
...
def load_meshes(body_v, body_f, garment_path):
    # Load body mesh
    ...
    #Load garment mesh
    garm_mesh = trimesh.load_mesh(garment_path)  # NOTE: Includes the texture
...
def render_images(body_v, body_f, garment_path, saved_path):

    pyrender_garm_mesh, pyrender_body_mesh = load_meshes(body_v, body_f, garment_path)

    # sides = ['front', 'back']
    # for side in sides:
    for i in tqdm(range(360), dynamic_ncols=True):
        saved_path_i = saved_path.replace('.png', f'_{i}.png')
        render(pyrender_garm_mesh, pyrender_body_mesh, i, saved_path_i)
